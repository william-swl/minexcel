...
def test_read_block_excel0(snapshot):
    ...
    res = mxl.read_block_excel(path, tmpl, skipheader=1).to_dict()
...
def test_read_block_excel1(snapshot):
    ...
    res = mxl.read_block_excel(path, tmpl, skipheader=1, intervalcols=1).to_dict()
    snapshot.assert_match(res)
...
def test_read_block_excel2(snapshot):
    ...
    res = mxl.read_block_excel(
        path, tmpl, skipheader=1, intervalrows=2, intervalcols=1
    ).to_dict()
    snapshot.assert_match(res)
